...
To dispatch Telegram-created issues to Copilot, enable the Copilot coding agent for the repository or organization, then use a dedicated bot account with a fine-grained PAT that has write access to `markjgardner/usmga-webadmin`. GitHub App installation tokens are not supported for this flow.
...
To re-engage Copilot after review or Telegram change requests, comment on the PR with `@copilot` and the requested changes. Copilot will update the same PR when repository settings and permissions allow it.
